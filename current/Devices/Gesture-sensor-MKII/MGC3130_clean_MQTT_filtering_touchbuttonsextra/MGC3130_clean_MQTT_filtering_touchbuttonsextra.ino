...
const int numButtons = 6;                                      // Number of touch buttons
const int touchPins[numButtons] = { T5, T6, T3, T2, T8, T7 };  // Arbitrary touch pins on ESP32 // T7 is a guse.
bool buttonState[numButtons] = { false };                      // Current state of each button
bool lastButtonState[numButtons] = { false };                  // Previous state of each button
...
void mqtt_callback(char* topic, byte* payload, unsigned int length) {
 // Serial.print("Message arrived [");
  if (!autohome.mqtt_callback(topic, payload, length)) {
    ...
    if (autohome.getValue(packet, ':', 0).equals("TT")) {           // change touch threshold, but its pritty good. 
      // colour
      TouchThreshold = autohome.getValue(packet, ':', 1).toInt();
      //changeColourRequested = true;
      //      TouchThreshold = packet.toInt();
...
void handleTouch() {
  ...
  for (int i = 0; i < numButtons; i++) {
    buttonState[i] = touchRead(touchPins[i]) < TouchThreshold;  // Adjust threshold as needed
  }
 // Serial.println(TouchThreshold);
 digitalWrite( 5, HIGH );
}
...
void setup() {
  ...
  for (int i = 0; i < numButtons; i++) {
    touchAttachInterrupt(touchPins[i], handleTouch, 40);  // Attach interrupt for each touch pin
...
void loop() {
  ...
  {
    // Check state changes for each button
    for (int i = 0; i < numButtons; i++) {
      if (buttonState[i] != lastButtonState[i]) {
        if (buttonState[i]) {
          ...
          String packet3 = ("Button touched " + String(i + 1));
          autohome.sendPacket(packet3.c_str());
        }
